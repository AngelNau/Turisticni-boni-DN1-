...
function izberiKorak(izbirniGumb) {
    ...
    switch (izbirniGumb.value) {
        case "osebe":
            document.querySelector('#osebe').style.backgroundColor = "#f2f2f2";
            document.querySelector('#parki').style.backgroundColor = "transparent";
            document.querySelector('#hoteli').style.backgroundColor = "transparent";
        ...
        case "parki":
            ...
            pridobiPodatke("parki", function (jsonRezultat, vrstaInteresneTocke) {
                document.querySelector('#osebe').style.backgroundColor = "transparent";
                document.querySelector('#parki').style.backgroundColor = "#f2f2f2";
                document.querySelector('#hoteli').style.backgroundColor = "transparent";
                parki = jsonRezultat;
        ...
        case "hoteli":
            document.querySelector('#osebe').style.backgroundColor = "transparent";
            document.querySelector('#parki').style.backgroundColor = "transparent";
            document.querySelector('#hoteli').style.backgroundColor = "#f2f2f2";
            prikaziHoteleZnotrajRadija();
            break;
    }
}
...
function prikaziHoteleZnotrajRadija() {
    ...
    preveriPogojeSklopaHoteli(function () {
        ...
        pridobiPodatke("hoteli", function (jsonRezultat, vrstaInteresneTocke) {
            ...
            radijHotelov = L.circle([srednjaTocka[1],srednjaTocka[0]], {
                radius: (document.querySelector("#radij").value*1000), 
                fillColor: "red", 
                stroke: false}).addTo(mapa);

            // Poišči vse hotele znotraj radija
            izracunajVseHoteleVRadiju(srednjaTocka[1],srednjaTocka[0], document.querySelector("#radij").value);
...
function izracunajVseHoteleVRadiju(lat, lng, radij) {
    ...
    for (let i = 0; i < hoteli.length; i++) {
        ...
        if(najblizjiHotelOddaljenost > oddaljenost) {
...
function preveriPogojeSklopaHoteli(callback) {
    ...
    if (document.querySelector("#odrasli").value == 0 && document.querySelector("#otroci").value && kandidatiParkov.length != 2) {
        sporocilo = "Skupni znesek v osnovnih podatkih mora biti več kot 0 €!\nIzbrati je potrebno 2 parka!";
        alert(sporocilo);
    } else if(kandidatiParkov.length != 2) {
        sporocilo = "Izbrati je potrebno 2 parka!";
        alert(sporocilo);
    } else if(document.querySelector("#odrasli").value == 0 && document.querySelector("#otroci").value == 0) {
        sporocilo = "Skupni znesek v osnovnih podatkih mora biti več kot 0 €!";
        alert(sporocilo);
    }
...
function dodajParkMedKandidate(indeks) {
    ...
    if (tabelaParkov.rows.length < 2 && kandidatiParkov[0] != indeks) {
